...
class Game:
    ...
    def draw(self):
        ...
        if self.game_state == "playing":
            ...
            death_line = ["Хотя, сбежал ты номинально,", f"ведь ты уже успел погибнуть целых {self.death_count} раз!"] if self.death_count != 0 else ["Да, ты действительно сбежал", "ни разу не умерев. Это круто!"]
            game_over_text = [
