...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        states = {
            ...
            "jump": {"flip": True},  # Автоматически создаст jump_flip
        }
    ...
    def handle_events(self, events):
        ...
        state = "jump" if not self.on_ground else "run" if self.velocity_x else "idle"
    ...
    def update(self, platforms, camera):
        ...
        for platform in platforms:
            if 'is_deadly' in platform and platform['is_deadly'] and self.rect.colliderect(platform['rect']):
                self.respawn()
                print(f"Hit deadly platform at {platform['rect']}")
        if self.on_ground and self.animator.current_state in ("jump", "jump_flip"):
            state = "idle" if self.velocity_x == 0 else "run"
            if not self.facing_right and self.velocity_x == 0:
                state += "_flip"
            self.animator.set_state(state)
    ...
    def draw(self, screen, camera=None):
        ...
        # Смещаем спрайт влево для компенсации смещения rect
